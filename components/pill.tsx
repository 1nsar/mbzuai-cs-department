...
export const Pill = ({ children, className }: { children: React.ReactNode, className?: string }) => {
  ...
  return (
    <div
      ...
      className={cn("bg-white/80 transform-gpu font-medium text-black backdrop-blur-xs font-mono text-sm inline-flex items-center justify-center px-3 h-8  border border-white/60 [clip-path:polygon(var(--poly-roundness)_0,calc(100%_-_var(--poly-roundness))_0,100%_var(--poly-roundness),100%_calc(100%_-_var(--poly-roundness)),calc(100%_-_var(--poly-roundness))_100%,var(--poly-roundness)_100%,0_calc(100%_-_var(--poly-roundness)),0_var(--poly-roundness))]", className)}
    >
      ...
      <span className="inline-block size-2.5 rounded-full bg-blue-600 mr-2 shadow-glow shadow-blue-600/50" />

  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        float baseAlpha = (1.04 - clamp(vDistance, 0.0, 1.0)) * clamp(smoothstep(-0.5, 0.25, vPosY), 0.0, 1.0);
        float alpha = baseAlpha * uOpacity * revealMask * uRevealProgress * sparkleBrightness;

        vec3 colorCyan = vec3(0.0, 0.8, 0.9); // Cyan color
        vec3 colorBlue = vec3(0.0, 0.3, 1.0); // Deep blue color
        
        // Create gradient based on world position (left to right)
        float gradientFactor = (vWorldPosition.x / 20.0) + 0.5; // Normalize to 0-1 range
        gradientFactor = clamp(gradientFactor, 0.0, 1.0);
        
        vec3 particleColor = mix(colorCyan, colorBlue, gradientFactor);

        gl_FragColor = vec4(mix(particleColor, vec3(sparkleBrightness), uTransition), mix(alpha, sparkleBrightness - 1.1, uTransition));
      }`,
      uniforms: {
